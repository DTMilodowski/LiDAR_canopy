...
for pp in range(0,N_plots):
    ...
    lidar_profiles[Plot_name] = np.sum(n,axis=1)
    ...
    lidar_profiles_adjusted[Plot_name] = np.sum(n,axis=1)
...
for pp in range(0,N_plots):
    ...
    for i in range(0,max_return):
        ax31.plot(lidar_profiles[Plot_name][:,i]/1000.,heights,'-',c=colour[i],linewidth=1)
        ax31.plot(lidar_profiles_adjusted[Plot_name][:,i]/1000.,heights,'--',c=color[i],linewidth=1)
    ax31.set_ylim(0,80)
