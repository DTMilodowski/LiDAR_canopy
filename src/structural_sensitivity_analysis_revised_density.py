...
for ss in range(0,sample_res.size):
    ...
    PAD_profiles_rad1[keys[ss]] = copy.deepcopy(temp_dic)
        ...
        for ss in range(0,sample_res.size):
            ...
            for pp in range(0,n_subplots):
                ...
                while np.all((nodata_test,radius<=50)):
                    ...
                    # now repeat but for adjusted profiles, accounting for
                    # imperfect penetration of LiDAR pulses into canopy
...
np.save("MH_density_sensitivity_%s.npy" % plot, PAD_profiles_MH)
np.save("MH_wt_density_sensitivity_%s.npy" % plot, PAD_profiles_MH_wt)
np.save("rad1_density_sensitivity_%s.npy" % plot, PAD_profiles_rad1)
np.save("rad2_density_sensitivity_%s.npy" % plot, PAD_profiles_rad2)
np.save("penetration_limit_density_sensitivity_%s.npy" % plot, penetration_limit)
