...
xmin = geoTransform[0]
xmax = xmin + float(cols+1)*geoTransform[1]

# check that y resolution is negative (standard formatting)
if geoTransform[5]<0:
    ymax = geoTransform[3]
    ymin = ymax + float(rows+1)*geoTransform[5]
else:
    ymin = geoTransform[3]
    ymax = ymin + float(rows+1)*geoTransform[5]
...
raster.write_raster_to_GeoTiff_UTM(pt_dens, geoTransform, ('%s_pointcloud_metrics_10m_point_density' % (site)), utm)
